...
[[ -f huffman/hm ]] || die "hm not found"
[[ -f utils/fbits ]] || die "fbits not found"
...
') <( ./utils/fbits ${TMPDIR}/fbits-test.txt 8 )"
...
ftell: 1 00000011
ftell: 2 00000111
' ) <(./utils/fbits ${TMPDIR}/fbits-test.txt 3 3 3)"
...
test_expect_to_succed 'All characters encode' "
	...
	python -c 'for i in range(256):print \"%c\" % i,' | ./huffman/hm > ${TMPDIR}/all.hu "

test_expect_to_succed 'All characters decompress' '
	./huffman/hm -d ${TMPDIR}/all.hu
'
...
l	1101
	...
	1110
 	1111
') <(echo -ne \"${TEST_TEXT}\" | ./huffman/hm -c 2>&-)"


test_expect_to_succed "huffman: encoding" "
	echo -ne \"${TEST_TEXT}\" | ./huffman/hm 2>&- > ${TMPDIR}/encoded.txt
...
test_expect_to_succed "huffman: decoding" "
	diff -Nur <( echo -ne \"${TEST_TEXT}\") <(./huffman/hm -d ${TMPDIR}/encoded.txt)
"

test_expect_to_succed 'huffman: random' "
	head -c 4096 /dev/urandom > ${TMPDIR}/random.txt
	./huffman/hm ${TMPDIR}/random.txt > ${TMPDIR}/random.hu
	diff -Nur ${TMPDIR}/random.txt <( ./huffman/hm -d ${TMPDIR}/random.hu)
"
